...
const transfer = ({
  ...
  asyncPipe(
    composePactCommand(
      execution(
        `(coin.transfer-crosschain "${sender}" "${receiver}" (read-keyset 'ks) "${chain}" ${amount})`,
      ),
      setMeta({
        chainId: '8',
      ...
      addSigner(
        'fab92c47ccb1fdd25173e2981ebb29de989acea88792e34cc87298f6a1578842',
        (withCap) => [
          withCap('coin.TRANSFER_XCHAIN', sender, receiver, amount, chain),
          withCap('coin.GAS'),
        ],
      ),
    ),
    async (tx) => {
      const spv = await l1Client.createSpv(tx, chain);
    },
  );
};
